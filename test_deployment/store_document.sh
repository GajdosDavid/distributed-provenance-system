...
curl --location "http://${IP}:8000/api/v1/organizations/ORG" \
...
TEMP_FILE=$(mktemp)
sed "s/PLACEHOLDER/${IP}/g" ./01_sample_acquisition.json > $TEMP_FILE
...
curl --location "http://${IP}:8000/api/v1/organizations/ORG/documents/01_sample_acquisition" \
     --header 'Content-Type: application/json' \
     --data "{
    \"document\": \"$DOCUMENT\",
    \"documentFormat\": \"json\",
